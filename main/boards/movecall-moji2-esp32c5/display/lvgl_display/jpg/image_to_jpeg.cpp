/**
 * @file image_to_jpeg.cpp
 * @brief image_to_jpeg.cpp 中各类和辅助函数的具体实现。
 */
#include <esp_attr.h>
...
/**
 * @brief 将 5 位颜色分量扩展为 8 位颜色分量。
 * @param v 低 5 位有效的颜色分量值，范围为 0 至 31。
 * @return 按位复制扩展后的 8 位颜色分量，范围为 0 至 255。
 */
...
/**
 * @brief 将 6 位颜色分量扩展为 8 位颜色分量。
 * @param v 低 6 位有效的颜色分量值，范围为 0 至 63。
 * @return 按位复制扩展后的 8 位颜色分量，范围为 0 至 255。
 */
...
/**
 * @brief 将输入图像转换为软件 JPEG 编码器可接受的像素布局。
 * @param src 输入图像首地址，数据长度必须与宽、高和像素格式相匹配。
 * @param width 图像宽度，单位为像素。
 * @param height 图像高度，单位为像素。
 * @param format 输入数据使用的 V4L2 像素格式。
 * @param out_fmt 输出参数，非空时写入软件编码器所需的像素格式。
...
/**
 * @brief 确保 ESP32 硬件 JPEG 编码引擎已创建。
 * @return 编码引擎可用时返回 true，创建失败时返回 false。
 * @details 该函数采用延迟初始化方式并复用全局编码句柄，避免每次截图都重复创建硬件引擎。
 */
...
/**
 * @brief 将输入图像整理为硬件 JPEG 编码器要求的内存布局。
 * @param src 输入图像首地址。
 * @param width 图像宽度，单位为像素。
 * @param height 图像高度，单位为像素。
 * @param format 输入数据使用的 V4L2 像素格式。
 * @param out_fmt 输出参数，非空时写入硬件编码器输入格式。
 * @param out_size 输出参数，非空时写入转换后缓冲区的字节数。
...
/**
 * @brief 尝试使用 ESP32 硬件编码器生成 JPEG 数据。
 * @param src 输入图像首地址。
 * @param src_len 输入图像字节数，用于描述输入缓冲区范围。
 * @param width 图像宽度，单位为像素。
 * @param height 图像高度，单位为像素。
 * @param format 输入数据使用的 V4L2 像素格式。
 * @param quality JPEG 质量，函数会将其限制在 1 至 100。
 * @param jpg_out 缓冲区输出模式下用于接收 JPEG 地址；回调模式下可为 nullptr。
 * @param jpg_out_len 缓冲区输出模式下用于接收 JPEG 字节数；回调模式下可为 nullptr。
 * @param cb 流式输出回调；非空时编码数据通过回调输出，并在结束后自动释放临时缓冲区。
...
/**
 * @brief 使用 esp_new_jpeg 软件编码器生成 JPEG 数据。
 * @param src 输入图像首地址。
 * @param src_len 输入图像字节数，用于描述输入缓冲区范围。
 * @param width 图像宽度，单位为像素。
 * @param height 图像高度，单位为像素。
 ...
 * @param jpg_out 缓冲区输出模式下用于接收 JPEG 地址；回调模式下可为 nullptr。
 * @param jpg_out_len 缓冲区输出模式下用于接收 JPEG 字节数；回调模式下可为 nullptr。
 * @param cb 流式输出回调；非空时编码结果通过该回调发送。
 * @param cb_arg 原样传递给 cb 的调用者上下文指针。
 * @return 输入转换和 JPEG 编码全部成功时返回 true，否则返回 false。
 * @details 本函数负责输入格式转换、编码器创建、输出缓冲区估算和所有失败路径的资源回收。
 */
...
/**
 * @brief 将一帧图像编码为连续的 JPEG 内存数据。
 * @param src 输入图像首地址，函数不会修改其中的数据。
 * @param src_len 输入图像字节数。
 * @param width 图像宽度，单位为像素。
 * @param height 图像高度，单位为像素。
 * @param format 输入数据使用的 V4L2 像素格式。
 * @param quality JPEG 质量，合法范围为 1 至 100。
 * @param out 输出参数，用于接收 JPEG 缓冲区地址。
 ...
 * @param format 输入数据使用的 V4L2 像素格式。
 * @param quality JPEG 质量，合法范围为 1 至 100。
 * @param cb 接收 JPEG 数据块和结束标记的回调函数，不得为空。
...
bool image_to_jpeg_cb(uint8_t* src, size_t src_len, uint16_t width, uint16_t height, v4l2_pix_fmt_t format,
    ...
    if (format == V4L2_PIX_FMT_JPEG) {
        ...
        cb(arg, 1, nullptr, 0); // 发送结束标记，通知调用者当前 JPEG 已输出完毕。
        return true;
    }
#endif // CONFIG_XIAOZHI_CAMERA_ALLOW_JPEG_INPUT
#if CONFIG_XIAOZHI_ENABLE_HARDWARE_JPEG_ENCODER
