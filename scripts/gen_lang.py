#!/usr/bin/env python3
"""把指定语言的 JSON 字符串和内嵌 OGG 符号生成 C++ 头文件。"""
import argparse
import json
import os

HEADER_TEMPLATE = """// 此文件由 scripts/gen_lang.py 自动生成，请勿手工修改
// 当前语言：{lang_code}；缺失条目回退到 en-US
#pragma once
...
namespace Lang {{
    ...
    // 字符串资源：当前语言缺少键时使用 en-US 对应值
    namespace Strings {{
{strings}
    }}

    // 音效资源：当前语言缺少文件时使用 en-US 对应音效
    namespace Sounds {{
{sounds}
    }}
}}
...
def load_base_language(assets_dir):
    """加载 assets_dir/locales/en-US/language.json；失败时返回空 strings 作为安全回退。"""
