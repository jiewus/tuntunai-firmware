...
固件已经接入囤囤AI的设备绑定、天气、备忘录和动态 MCP 接口。当前实现范围如下：
...
工具没有输入参数。只有用户明确提出绑定或连接囤囤AI平台时，大模型才应调用该工具。
例如用户可以说：
...
用户明确要求解绑或删除当前设备时，小智通过该工具告知用户登录囤囤AI后台，在设备管理中完成
...
用户登录囤囤AI网页端，在设备页面输入屏幕上的短绑定码。Web 调用：
...
天气和天气设置响应中的 `location_name` 是面向客户端显示的短名称。后端仍在数据库中保留完整行政区
和编码用于查询，但响应时会提取末级位置并去掉“市”“区”“县”等行政区类型后缀；例如
“安徽省合肥市”“新疆维吾尔自治区乌鲁木齐市”和“上海市松江区”分别返回“合肥”“乌鲁木齐”和
“松江”。固件写入屏保前执行相同的兼容处理，以支持仍返回标准行政区全称的旧服务或缓存数据。
...
- 设备 Token 缺失时显示“请先绑定囤囤AI”；返回 `401` 或 `403` 且没有历史缓存时显示
  “设备认证已失效”。
...
固件不周期轮询清单，也不会主动调用动态 MCP 工具。工具只会在小智实际发起 MCP `tools/call` 时
执行；自动执行计划由囤囤AI服务器 Worker 负责，不由设备固件负责。
...
用户询问“我有几个自定义工具包”“我有哪些自定义 MCP”“显示自定义工具列表”等问题时，大模型必须
调用该工具。工具只读取设备最近一次成功安装的动态 MCP 内存快照，不请求后端、不执行清单中的任何
动态工具，也不把固件内置工具计入数量。

工具返回当前动态 MCP 总数，并按照清单顺序逐项提供名称和后台配置的说明，由小智正常完成 TTS 播报。
屏幕和语音中的名称会去除固定 `custom.` 前缀；工具实际注册和执行仍使用包含前缀的完整名称。
没有已加载工具时，屏幕和语音均明确提示暂无自定义 MCP。

调用后圆屏切换到自定义 MCP 清单专用表盘。该页面隐藏天气、日期、时间、备忘录、Wi-Fi 和电量，
只保留金属黑背景、外圈和刻度。30px 标题固定在页面顶部；下方圆角区域每次只显示一个 MCP，名称和
说明使用 25px 粗体并按实际高度垂直居中，过长说明在当前单项区域内使用省略号截断。存在多个工具时
由 5 秒定时器按照清单顺序切换，不执行连续滚动，因此不会持续占用 Opus 解码所需的单核 CPU 和显示
总线。语音播报结束并离开 speaking 状态时立即退出该页面，恢复小智聊天界面；对话结束进入 idle 时
继续由原有状态机显示普通屏保。
