...
创建工具接收必填 `content` 和可选 `remind_at`。有明确提醒时间时，大模型必须把自然语言时间转换为
带 `+08:00` 偏移的 RFC 3339 绝对时间；没有提醒时间时省略该参数或传空字符串。查询工具的 `status`
...
修改后的完整正文、提醒时间和状态；删除工具仅在用户明确要求删除时调用。数量类问题使用统计工具，
避免模型根据分页结果自行计算。
...
每条屏保备忘录固定显示三行，日期时间在第一行，正文在下面两行，超出部分使用省略号截断。提醒时间
为今天时显示 `HH:mm`；属于今年但不是今天时显示 `MM-dd HH:mm`；不属于今年时显示
`yyyy-MM-dd` 且不显示时间；没有提醒时间时显示“未设置时间”。
