...
void WifiStation::HandleScanResult() {
    ...
    for (int i = 0; i < ap_num; i++) {
        ...
        if (it != ssid_list.end()) {
            ESP_LOGI(TAG, "发现候选接入点，SSID=%s，BSSID=%02x:%02x:%02x:%02x:%02x:%02x，信号=%d，信道=%d，认证模式=%d",
                (char *)ap_record.ssid,
    ...
    if (connect_queue_.empty()) {
        ESP_LOGI(TAG, "未发现可连接接入点，将在%d秒后重新扫描", scan_current_interval_microseconds_ / 1000 / 1000);
...
int8_t WifiStation::GetRssi() {
    ...
    if (err != ESP_OK) {
        ESP_LOGW(TAG, "获取接入点信息失败，原因=%s", esp_err_to_name(err));
        return 0;
    }
    return ap_info.rssi;
}
...
void WifiStation::WifiEventHandler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data) {
    ...
    if (event_id == WIFI_EVENT_STA_START) {
        ...
        ESP_LOGI(TAG, "Wi-Fi 已断开，原因码=%d", event->reason);
        ...
        if (this_->reconnect_count_ < MAX_RECONNECT_COUNT) {
            ...
            ESP_LOGI(TAG, "正在重连 Wi-Fi，SSID=%s，第%d/%d次", this_->ssid_.c_str(), this_->reconnect_count_, MAX_RECONNECT_COUNT);
            return;
        }
        ...
        ESP_LOGI(TAG, "没有更多可连接接入点，将在%d秒后重新扫描",
...
void WifiStation::IpEventHandler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data) {
    ...
    ESP_LOGI(TAG, "Wi-Fi 已获取 IP，地址=%s", this_->ip_address_.c_str());
