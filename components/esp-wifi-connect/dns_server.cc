...
void DnsServer::Start(esp_ip4_addr_t gateway) {
    ...
    if (bind(fd_, (struct sockaddr *)&server_addr, sizeof(server_addr)) < 0) {
        ESP_LOGE(TAG, "DNS 端口绑定失败，端口=%d", port_);
        close(fd_);
        fd_ = -1;
        return;
    }
...
void DnsServer::Run() {
    ...
    while (running_) {
        ...
        if (len < 0) {
            ...
            ESP_LOGE(TAG, "DNS 请求接收失败，错误码=%d", errno);
            continue;
        }
        ...
        len += 4;
        ESP_LOGI(TAG, "正在发送 DNS 响应，目标=%s", inet_ntoa(gateway_.addr));

        sendto(fd_, buffer, len, 0, (struct sockaddr *)&client_addr, client_addr_len);
    }

    task_handle_ = nullptr;
    ESP_LOGI(TAG, "配网 DNS 任务已退出");
}
