...
bool WifiManager::Initialize(const WifiManagerConfig& config) {
    ...
    if (initialized_) {
        ESP_LOGW(TAG, "Wi-Fi 管理器已经初始化");
        return true;
    }

    config_ = config;
    ESP_LOGI(TAG, "正在初始化 Wi-Fi 管理器");

    // Initialize NVS
    esp_err_t ret = nvs_flash_init();
    if (ret == ESP_ERR_NVS_NO_FREE_PAGES || ret == ESP_ERR_NVS_NEW_VERSION_FOUND) {
        ESP_LOGW(TAG, "NVS 空间不足或版本变化，正在擦除");
        ESP_ERROR_CHECK(nvs_flash_erase());
        ret = nvs_flash_init();
    }
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "NVS 初始化失败，原因=%s", esp_err_to_name(ret));
        return false;
    }
    ...
    if (ret != ESP_OK && ret != ESP_ERR_INVALID_STATE) {
        ESP_LOGE(TAG, "网络接口初始化失败，原因=%s", esp_err_to_name(ret));
        return false;
    }
    ...
    if (ret != ESP_OK && ret != ESP_ERR_INVALID_STATE) {
        ESP_LOGE(TAG, "默认事件循环创建失败，原因=%s", esp_err_to_name(ret));
        return false;
    }
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Wi-Fi 驱动初始化失败，原因=%s", esp_err_to_name(ret));
        return false;
    }
...
void WifiManager::StartStation() {
    ...
    if (!initialized_) {
        ESP_LOGE(TAG, "Wi-Fi 管理器尚未初始化");
        return;
    }
    if (station_active_) {
        ESP_LOGW(TAG, "Wi-Fi 终端模式已经启动");
        return;
    }

    // Auto-stop config AP if active
    if (config_mode_active_) {
        ESP_LOGI(TAG, "启动终端模式前正在停止配网热点");
        config_ap_->Stop();
...
void WifiManager::StopStation() {
    ...
    ESP_LOGI(TAG, "正在停止 Wi-Fi 终端模式");
    station_->Stop();
    ESP_LOGI(TAG, "Wi-Fi 终端模式已停止");
...
void WifiManager::StartConfigAp() {
    ...
    if (config_mode_active_) {
        ESP_LOGW(TAG, "配网热点已经启动");
        return;
    }

    // Auto-stop station if active
    if (station_active_) {
        ESP_LOGI(TAG, "启动配网热点前正在停止终端模式");
        station_->Stop();
        station_active_ = false;
        lock.unlock();
        NotifyEvent(WifiEvent::Disconnected);
